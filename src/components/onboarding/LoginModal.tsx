...
export default function LoginModal() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center backdrop-blur-sm bg-black/50">
      <div className={`w-full max-w-sm mx-4 rounded-2xl border shadow-2xl p-8 ${isDark ? "bg-slate-900/95 border-white/10" : "bg-background/95 border-border/50"}`}>
        <div className="text-center space-y-6">
          <div className="space-y-2">
            ...
            <h1 className={`text-2xl font-serif ${isDark ? "text-white" : "text-foreground"}`}>MoodTune</h1>
            <p className={`text-sm ${isDark ? "text-white/60" : "text-muted-foreground"}`}>天気と時間に合わせた音楽プレイリスト</p>
          </div>

          <div className="space-y-3">
            <p className={`text-sm ${isDark ? "text-white/70" : "text-foreground/70"}`}>
