...
3. **開発効率**: Mood Tuning により、開発中に様々な天気・時間帯の組み合わせを簡単にテストできる

## ユーザージャーニー（現在の実装）

1. アプリを開く
2. （未ログイン時）ログインモーダルで「Spotifyでログイン」または「ログインせずに使う」を選択
3. 位置情報の許可（初回のみ）
4. 現在の天気と時間に応じた背景色・アニメーションが自動表示
5. **Select Genre**（Settings 内）で好みのジャンルを最大 4 個選択（保存は自動）
...
9. 気に入ったプレイリストを視覚的に選択。「Spotifyで再生」で MoodTune プレイリストを保存して Spotify アプリで再生可能。（**今後のビジョン**: アプリ内での音楽再生）
