...
3. **プレイリスト探索 UI**
   ...
   - **Select Genre パネル**（Settings 内）: 21 ジャンルから最大 4 個選択可能。選択は localStorage に永続化。0 件はセッション内のみ許可（警告表示・パネル閉じ不可）。読み込み時に空配列はデフォルトに修復
...
4. **Mood Tuning パネル**（左下）
   ...
   - 実時刻・実天気の変化時は自動更新（Mood Tuning 中は対象外）

5. **認証**
   - Spotify PKCE（Authorization Code with PKCE）で認証。未ログインでも利用でき、ログインモーダルで「ログインせずに使う」を選択可能
