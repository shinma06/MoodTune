...
1. **天気連動システム**
   ...
   - 14 種類の天気タイプに対応（Clear, Clouds, Rain, Drizzle, Thunderstorm, Snow, Mist, Fog, Haze, Dust, Sand, Ash, Squall, Tornado）
   - 時間帯（朝/昼/夕方/夜）に応じた背景色の動的変化

2. **視覚的フィードバック**

   - 天気と時間帯に応じたグラデーション背景（4 色: top, from, via, to）
...
3. **プレイリスト探索 UI**
   ...
   - 5 つのプレイリスト（チルアウト、アンビエント、クラシック、ローファイヒップホップ、アコースティック）

4. **開発・テスト機能**
   - メイン UI 上にテストパネル（右上の設定アイコン）
   - 天気タイプと時間帯を手動で設定可能
   - 選択時に即座に反映（適用ボタン不要）
   - リセットで実際の天気に戻る
