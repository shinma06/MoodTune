...
- Mood Tuning パネルの UI 統一:
  ...
  - 展開パネルの下端位置を微調整し、トグルボタンとの間に小さな余白を確保（`bottom-24` → `bottom-26`）
  - トグルボタンの「展開中」背景色・ホバー色を `isOverlayThemeDark` に連動（ダーク時は白系、ライト時は primary 系）
  - トグルボタンの「展開前（クローズ時）」ホバー色・境界色も `isOverlayThemeDark` に連動
- 初期同期時のローディング文言:
