...
- 都市名取得を Google Geocoding API（逆ジオコーディング）に移行:
  ...
  - `fetchWeatherData`: 天気（OpenWeatherMap）と Geocoding を `Promise.all` で並列取得。都市名は Geocoding の `city` を優先し、失敗・空の場合は OpenWeatherMap の `name` にフォールバック
  - 天気APIは従来どおり `NEXT_PUBLIC_WEATHER_API_KEY`（OpenWeatherMap）のまま
